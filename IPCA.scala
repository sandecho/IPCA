...
class IPCA(val k: Int, val b: Int) extends Serializable {
  ...
  require(b >= k,
    s"The batch size must be greater than or equal to the Number of principal components but got ${b}")
  ...
  /**
   * Computes a [[PCAModel]] that contains the principal components of the input vectors using incremental approach.
   *
  ...
  def fit(sources: RDD[Vector]): PCAModel = {
    ...
    while (i < numPartition) {
      ...
      if (numSampleSeen == 0) {
        ...
        rows = rows.map { x => { subtract(x, col_mean) } }
        var mean_correction = scalarMultiply(Math.sqrt((numSampleSeen * size) / (numSampleSeen + size)), subtract(mean, col_mean))
        var j = -1
      ...
      val svd: SingularValueDecomposition[RowMatrix, Matrix] = mat.computeSVD(k.toInt, computeU = false)
      val s: Vector = svd.s
      ...
      if (i == numPartition) {
        ...
        val explainedVariance = explainedVariance1 match {
          case dv: DenseVector =>
            dv
